...
import { MDS_BREADCRUMBS, BREADCRUMBS, DEFAULT_OUTPUT_AFTER } from '../../../utils/constants';
import { CoreServicesContext } from '../../../components/CoreServices/CoreServices';
import { getDataSourceEnabled, getDataSourceManagementPlugin, getSavedObjectsClient, getNotifications } from '../../../services';
...
const [visualizationOptions, setVisualizationOptions] = useState<VisualizationOptions>({
  ...
  const formatTime = (timeField: string) => {
  ...
  const getStateMsg = () => {
    ...
    switch (forecaster?.curState) {
      ...
      case FORECASTER_STATE.INACTIVE_STOPPED: {
        return 'Stopped at ' + formatTime('realTimeLastUpdateTime');
      }

      case FORECASTER_STATE.AWAITING_DATA_TO_INIT: {
        return 'Has been awaiting sufficient data to initialize forecast since ' + formatTime('realTimeLastUpdateTime');
      }

      case FORECASTER_STATE.AWAITING_DATA_TO_RESTART: {
        return 'Has been awaiting sufficient data to restart forecast since  ' + formatTime('realTimeLastUpdateTime');
      }

      case FORECASTER_STATE.INIT_TEST: {
        return 'Initializing test since ' + formatTime('runOnceLastUpdateTime');
      }

      case FORECASTER_STATE.INITIALIZING_FORECAST: {
        return 'Initializing forecast since ' + formatTime('realTimeLastUpdateTime');
      }

      case FORECASTER_STATE.TEST_COMPLETE: {
        return 'Test complete at ' + formatTime('runOnceLastUpdateTime');
      }

      case FORECASTER_STATE.RUNNING: {
        return 'Running since ' + formatTime('realTimeLastUpdateTime');
      }
  ...
  const forecasterIntervalInMin = get(
    forecaster,
    'forecastInterval.period.interval',
    1
  );
            ...
            <Fragment>
              ...
              >
                ...
                <EuiDescribedFormGroup
                  ...
                  description="Select the data source and focus the relevant data with filters."
    ...
    <EuiPage paddingSize="m">
      <EuiPageBody panelled={false}>
        <EuiPageHeader style={{ justifyContent: 'space-between' }}>
          {/* Data Source Component - on top of the page */}
          {dataSourceEnabled && renderDataSourceComponent}

          {/* Left Section: Forecaster Name & Status */}
          <EuiPageHeaderSection>
            <EuiTitle size="m">
              <h2 style={{ margin: 0 }}>
                {forecaster?.name ?? ''}
                {getStateMsg() && (
                  ...
                  >
                    <EuiHealth
                      color={forecastStateToColorMap.get(forecaster?.curState)}
          ...
          {/* Right Section: Trash Icon + Start/Cancel Test + Start/Cancel Forecasting */}
          <EuiPageHeaderSection>
            <EuiFlexGroup
              gutterSize="s"
              alignItems="center"
            ...
            >
              ...
              {/* Test Button */}
              {/* Only show test button when state is NOT active */}
              {!isActiveState(forecaster?.curState) && (
                <EuiFlexItem grow={false}>
                  <EuiSmallButton
                    data-test-subj="startCancelTestButton"
                    isDisabled={forecaster?.curState === FORECASTER_STATE.INIT_TEST || runOnceRunning.current}
